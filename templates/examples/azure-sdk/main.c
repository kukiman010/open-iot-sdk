    ...
    while (1) {
        ...
        switch (msg.event) {
            ...
            case APP_EVENT_SEND_MSG_OK:
                msg_counter++;
                PW_LOG_INFO("Send message %" PRIu32 " of %d", msg_counter, MESSAGE_NUMBER);
            ...
            case APP_EVENT_IOT_HUB_CONNECTION_DOWN:
                PW_LOG_ERROR("IoT Hub connection failed %" PRId32 "; reconnecting", msg.return_code);
                // Connection is down, the middleware will attempt to reconnect
                break;
            case APP_EVENT_SEND_MSG_ERROR:
                PW_LOG_ERROR("Send message failed %" PRId32, msg.return_code);
                res = EXIT_FAILURE;
                break;
            default:
                break;
