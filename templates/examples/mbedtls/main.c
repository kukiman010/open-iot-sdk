...
static int mbedtls_iot_socket_send(void *ctx, const unsigned char *buf, size_t len)
{
    ...
    int32_t res = iotSocketSend(context->socket_fd, buf, len);
    if (res < 0) {
...
static int mbedtls_iot_socket_recv(void *ctx, unsigned char *buf, size_t len)
{
    app_context_t *context = (app_context_t *)ctx;
    ...
    int32_t res = iotSocketRecv(context->socket_fd, buf, len);
    if (res < 0) {
        return iot_socket_to_mbedlts_error(res);
    }
...
static int connect_tls(void)
{
    ...
    if (flags != 0) {
        ...
        if (res < 0) {
            ...
            printf("Certificate verification failed (flags %" PRIu32 "):"
                   "\n%s\r\n",
                   flags,
                   appContext.gp_buf);
            return flags;
