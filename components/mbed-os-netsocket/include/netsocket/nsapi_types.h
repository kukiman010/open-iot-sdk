...
enum nsapi_error {
    ...
    NSAPI_ERROR_HOST_NOT_FOUND = -3021,     /*!< Host not found*/
    NSAPI_ERROR_UNKNOWN = -3022,            /*!< Unknown error*/
};
...
typedef enum nsapi_event {
    NSAPI_EVENT_CONNECTION_STATUS_CHANGE =
        0, /*!< network connection status has changed, the parameter = new status (nsapi_connection_status_t) */
    NSAPI_EVENT_CELLULAR_STATUS_BASE =
        0x1000, /*!< Cellular modem status has changed, See the enum values from enum cellular_connection_status_t in
                   /connectivity/cellular/framework/common/CellularCommon.h */
    NSAPI_EVENT_CELLULAR_STATUS_END =
        0x1FFF /*!< cellular modem status has changed, See the enum values from enum cellular_connection_status_t in
                  /connectivity/cellular/framework/common/CellularCommon.h */
} nsapi_event_t;
 ...
 *  @enum nsapi_ip_stack
 */
typedef enum nsapi_ip_stack { DEFAULT_STACK = 0, IPV4_STACK, IPV6_STACK, IPV4V6_STACK } nsapi_ip_stack_t;
...
typedef struct nsapi_stack_api {
     ...
     *  @return         0 on success, negative error code on failure
     */
    nsapi_error_t (*socket_accept)(
        nsapi_stack_t *stack, nsapi_socket_t server, nsapi_socket_t *socket, nsapi_addr_t *addr, uint16_t *port);
     ...
     *                  code on failure
     */
    nsapi_size_or_error_t (*socket_send)(nsapi_stack_t *stack,
     ...
     *                  code on failure
     */
    nsapi_size_or_error_t (*socket_sendto)(nsapi_stack_t *stack,
                                           nsapi_socket_t socket,
                                           nsapi_addr_t addr,
                                           uint16_t port,
                                           const void *data,
     ...
     *                  code on failure
     */
    nsapi_size_or_error_t (*socket_recvfrom)(nsapi_stack_t *stack,
                                             nsapi_socket_t socket,
                                             nsapi_addr_t *addr,
                                             uint16_t *port,
                                             void *buffer,
    ...
    // TODO: Documentation
    nsapi_size_or_error_t (*socket_sendto_control)(nsapi_stack_t *stack,
                                                   nsapi_socket_t socket,
                                                   nsapi_addr_t addr,
                                                   uint16_t port,
                                                   const void *data,
                                                   nsapi_size_t size,
                                                   const nsapi_msghdr_t *control,
                                                   nsapi_size_t control_size);

    nsapi_size_or_error_t (*socket_recvfrom_control)(nsapi_stack_t *stack,
                                                     nsapi_socket_t socket,
                                                     nsapi_addr_t *addr,
                                                     uint16_t *port,
                                                     void *data,
    ...
    nsapi_error_t (*setsockopt)(
        nsapi_stack_t *stack, nsapi_socket_t socket, int level, int optname, const void *optval, unsigned optlen);
     ...
     *  @return         0 on success, negative error code on failure
     */
    nsapi_error_t (*getsockopt)(
        nsapi_stack_t *stack, nsapi_socket_t socket, int level, int optname, void *optval, unsigned *optlen);
} nsapi_stack_api_t;
