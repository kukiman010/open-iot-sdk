...
#ifdef DEFAULT_IPv6
#define DEFAULT_IP_VERSION NSAPI_IPv6
#else
#define DEFAULT_IP_VERSION NSAPI_IPv4
#endif
...
public:
     ...
     */
    explicit SocketAddress(const nsapi_addr_t &addr, uint16_t port = 0);
     ...
     *  @param port     Optional 16-bit port, defaults to 0
     */
    explicit SocketAddress(const char *addr, uint16_t port = 0);
     ...
     *  @param port     Optional 16-bit port, defaults to 0
     */
    explicit SocketAddress(const void *bytes, nsapi_version_t version, uint16_t port = 0);

    /** Create a SocketAddress from another SocketAddress
     *
     *  @param addr  SocketAddress to copy
     */
    explicit SocketAddress(const SocketAddress &addr);
