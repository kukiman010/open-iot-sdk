 ...
 */
static inline uint8_t *common_write_16_bit(uint16_t value, uint8_t ptr[2])
{
    *ptr++ = value >> 8;
    *ptr++ = value;
    return ptr;
...
bool stoip6(const char *ip6addr, size_t len, void *dest)
{
    ...
    if (len >= MAX_IPV6_STRING_LEN_WITH_TRAILING_NULL) { // Too long, not possible. We do not support IPv4-mapped IPv6
                                                         // addresses
        goto error;
    }
