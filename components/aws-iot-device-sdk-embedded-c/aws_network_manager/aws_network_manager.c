...
static int32_t mbedtls_to_mqtt_error(int error_code);
...
int32_t AwsNetwork_send(NetworkContext_t *ctx, const void *buf, size_t len)
{
    if (!ctx) {
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }

    int res = mbedtls_ssl_write(&ctx->ssl, (const unsigned char *)buf, len);

    return mbedtls_to_mqtt_error(res);
}

int32_t AwsNetwork_recv(NetworkContext_t *ctx, void *buf, size_t len)
{
    if (!ctx) {
        return MBEDTLS_ERR_SSL_BAD_INPUT_DATA;
    }

    int res = mbedtls_ssl_read(&ctx->ssl, (unsigned char *)buf, len);
...
static int32_t mbedtls_to_mqtt_error(int return_code)
{
    if (return_code < 0) {
        switch (return_code) {
            ...
            case MBEDTLS_ERR_SSL_WANT_READ:
                return 0;
            default:
                PW_LOG_ERROR("Received error code %d from mbed tls", return_code);
                break;
        }
    }

    return (int32_t)return_code;
}
...
static int iot_socket_send_bio(void *bio, const unsigned char *buf, size_t len)
{
    ...
    if (res < 0) {
        if (res == IOT_SOCKET_EAGAIN) {
            ...
            PW_LOG_ERROR("iotSocketSend returned : %" PRId32, res);
...
static int iot_socket_recv_bio(void *bio, unsigned char *buf, size_t len)
{
    ...
    if (res < 0) {
        if (res == IOT_SOCKET_EAGAIN) {
            ...
            PW_LOG_ERROR("iotSocketRecv returned : %" PRId32, res);
            return iot_socket_to_mbedlts_error(res);
        }
    }
