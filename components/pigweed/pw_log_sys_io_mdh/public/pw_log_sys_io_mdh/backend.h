 ...
 */
#ifndef PW_LOG_SYS_IO_MDH_BACKEND_H_
#define PW_LOG_SYS_IO_MDH_BACKEND_H_
...
void _pw_log_sys_io_mdh(const char *message, ...) PW_PRINTF_FORMAT(1, 2);

// Private function used to convert the log level into the right string.
static inline const char *_pw_log_sys_io_mdh_level_name(int level)
{
    switch (level) {
...
#define PW_LOG_MDH_SUFFIX "\r" // Newline appended by pw::sys_io::WriteLine

// Pigweed log backend implementation
#define PW_HANDLE_LOG(level, flags, LOG_MESSAGE, ...)                                                                \
    do {                                                                                                             \
        _pw_log_sys_io_mdh(PW_LOG_MDH_PREFIX LOG_MESSAGE PW_LOG_MDH_SUFFIX PW_COMMA_ARGS(PW_LOG_MDH_TIMESTAMP_VALUE) \
                               PW_COMMA_ARGS(_pw_log_sys_io_mdh_level_name(level)) PW_COMMA_ARGS(PW_LOG_MODULE_NAME) \
...
#endif /* PW_LOG_SYS_IO_MDH_BACKEND_H_ */
