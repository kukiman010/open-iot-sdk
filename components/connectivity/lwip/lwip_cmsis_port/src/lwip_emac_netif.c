...
{
    size_t mtu_size = mdh_emac_get_mtu(context->emac);
    mdh_network_stack_memory_manager_t *memory_manager = lwip_mm_get_instance();
    uint8_t *rx_buffer = context->rx_buffer;

    // process all buffers received by the emac driver until we get back 0 bytes
    do {
        // reset reception buffer
        mdh_network_stack_memory_manager_set_payload_len(memory_manager, rx_buffer, mtu_size);

        mdh_emac_status_t status = mdh_emac_receive(context->emac, rx_buffer);

        if (status != MDH_EMAC_STATUS_NO_ERROR
            || mdh_network_stack_memory_manager_get_payload_len(memory_manager, rx_buffer) == 0) {
            break;
        }

        struct pbuf *buf = mdh_network_stack_memory_manager_alloc_from_static_pool(
            memory_manager,
            mdh_network_stack_memory_manager_get_payload_len(memory_manager, rx_buffer),
            mdh_emac_get_align(context->emac));
        ...
        mdh_network_stack_memory_manager_copy(memory_manager, rx_buffer, buf);

        /* pass all packets to ethernet_input, which decides what packets it supports */
        if (context->lwip_netif.input(buf, &context->lwip_netif) != ERR_OK) {
            LWIP_DEBUGF(NETIF_DEBUG, ("EMAC LWIP: lwip rejected packet from EMAC\n"));
            mdh_network_stack_memory_manager_free(memory_manager, buf);
        }
    } while (true);
}
...
err_t ethernetif_init(struct netif *netif)
{
    ...
    context->rx_buffer = mdh_network_stack_memory_manager_alloc_from_heap(
        memory_manager, mdh_emac_get_mtu(context->emac), mdh_emac_get_align(context->emac));
    ...
    if (MDH_EMAC_STATUS_NO_ERROR
        ...
        mdh_network_stack_memory_manager_free(memory_manager, context->rx_buffer);
        return ERR_IF;
    }
