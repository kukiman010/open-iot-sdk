...
{
    int ret = osEventFlagsWait(event_flags_id, event, osFlagsWaitAny | osFlagsNoClear, AZURE_EVENT_TIMEOUT);
    if (ret < 0) {
        LogError("osEventFlagsWait failed %d", ret);
        return MU_FAILURE;
    }
    // we need to clear the selected flags manually because the osEventFlagsWait would clear all of them
    osEventFlagsClear(event_flags_id, ret & event);
    return 0;
}
