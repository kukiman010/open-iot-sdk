...
TEST_F(TestTickcounterOpeniot, tickcounter_create_returns_valid_ptr)
{
    ...
    EXPECT_EQ(reinterpret_cast<TICK_COUNTER_HANDLE>(fake_heap_tick_counter_memory), tick_counter);
}
...
TEST_F(TestTickcounterOpeniot, tickcounter_destroy_calls_free_if_tickcounter_is_not_null)
{
    ...
    // so that dereferences do not result in a crash
    uint8_t fake_heap_tick_counter_memory[100] = {0};
    tickcounter_destroy(reinterpret_cast<TICK_COUNTER_HANDLE>(fake_heap_tick_counter_memory));
...
TEST_F(TestTickcounterOpeniot, tickcounter_get_current_ms_returns_failure_when_current_ms_is_null)
{
    ...
    EXPECT_EQ(
        MU_FAILURE,
        tickcounter_get_current_ms(reinterpret_cast<TICK_COUNTER_HANDLE>(fake_heap_tick_counter_memory), nullptr));
}
...
TEST_F(TestTickcounterOpeniot, tickcounter_get_current_ms_returns_incremented_ms)
{
    ...
    EXPECT_NE(
        MU_FAILURE,
        tickcounter_get_current_ms(reinterpret_cast<TICK_COUNTER_HANDLE>(fake_heap_tick_counter_memory), &current_ms));
